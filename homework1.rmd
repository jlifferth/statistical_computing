...
```
"Highest coefficient: GenderM = 4.508675
```
...
pt(2.917, df=7, lower.tail = FALSE)
```
...
```{r}
print(sum(residuals(mod)**2))

deviance(mod)
```
Spoiler alert: the values were the same!
...
```{r}
(deviance(mod) / df.residual(mod)) ** .5
```
