...
for(i in 1:num_bootstrap_intervals) {
  ...
  for(j in 1:num_bootstrap_samples) {
    ...
    # group 0 stats
    mean0 <- mean(subset(df_combined, treatment == 0)$value)
    quant0 <- quantile(subset(df_combined, treatment == 0)$value, c(.025, .975))
    lower0 <- as.double(quant0[1])
    upper0 <- as.double(quant0[2])
    quant0_bootstraps[j,] <- data.frame(mean=mean0, upper=upper0, lower=lower0)
    
    # group 1 stats
    mean1 <- mean(subset(df_combined, treatment == 1)$value)
    quant1 <- quantile(subset(df_combined, treatment == 1)$value, c(.025, .975))
...
ggplot() + geom_line(aes(x=1:length(quantiles_0$mean),y=quantiles_0$mean, color='Group 1')) + geom_line(aes(x=1:length(quantiles_1$mean),y=quantiles_1$mean, color='Group 0')) + 
  ...
  ggtitle("Bootstrap intervals by treatment group") +
  geom_line() + 
  labs()
